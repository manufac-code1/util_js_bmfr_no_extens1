...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("data/full_bookmarks.json").then(response => response.json()).then(data => {
    console.log("Fetched data:", data);

    // Ensure each node has a unique ID
    function ensureUniqueIds(nodes, parentId = null, depth = 0, logCount = {
      count: 0
    }) {
      return nodes.filter((node, index) => {
        if (logCount.count < 30) {
          // Limit logging to the first 30 nodes
          console.log(`Processing node: ${node.id}, parentId: ${parentId}, relative position: ${index}`);
          logCount.count++;
        }
        ...
        return {
          ...node,
          id: node.id || uniqueId,
          children: node.children ? ensureUniqueIds(node.children, uniqueId, depth + 1, logCount) : node.children,
          parent: parentId
        };
      });
    }
    ...
    let rootNodes = dataWithUniqueIds.find(node => node.id === "0").children;

    // Directly filter out the specific node
    rootNodes = rootNodes.filter(node => !(node.parentId === "1" && node.url === "chrome://bookmarks"));
      ...
      return nodes.map(node => {
        if (node.id === path[0]) {
          return {
            ...
            children: node.children ? markNodesAsOpened(node.children, path.slice(1)) : node.children
          };
        }
        return node;
      });
